...
class GridUnstructuredArchive(GridArchive):
    """An archive that divides each dimension into uniformly-sized cells
    and can continue to grow without bounds.

    This unstructured variant of the :class:`GridArchive` allows the bounds
    on the dimensions of measure space to grow to include measures outside
    the original space. When this occurs, a new cell having the same size as
    the initial cell size is added. This way even as the archive grows all
    the cells remain the same size - the only thing that changes is the
    number of cells in each dimension of measure space.

    This archive is the unstructured version of the container described in
    `Mouret 2015 <https://arxiv.org/pdf/1504.04909.pdf>`_. It can be visualized
    as an n-dimensional grid in the measure space that is divided into a
    certain number of cells in each dimension. Each cell contains an elite,
    i.e. a solution that `maximizes` the objective function for the measures in
    that cell.
    ...
    def index_of(self, measures):
        ...
        # resize the archive if necessary
        if np.any(np.logical_or(grid_indices < 0, grid_indices >= self._dims)):
            # We don't care about anywhere the new min is positive
            new_min = grid_indices.min(axis=0)
            new_min[new_min > 0] = 0

            # We don't care about anywhere the new max is below the old max
            # Since we are zero-indexed the new max is `max_index + 1`
            new_max = grid_indices.max(axis=0)
            new_max = np.maximum(grid_indices.max(axis=0) + 1, self._dims)

            # Shift the bounds by whole intervals and update the interval size
            # and dims by the new endpoints.
            self._lower_bounds += new_min * self._interval_size / self._dims
            self._upper_bounds += (
                new_max - self._dims) * self._interval_size / self._dims
            self._interval_size = self._upper_bounds - self._lower_bounds
            self._dims = new_max - new_min
            self._boundaries = [
                np.linspace(lower_bound, upper_bound, dim + 1)
            ...
            # shift the calculated indices to the right by the number of new
            # cells added to the left
            grid_indices -= new_min

            # resize the store and move solutions so they appear at the shifted
            # indices
            self._store.resize(np.prod(self._dims))
            if np.any(new_min < 0):
                # since we set all positive entries of `new_min` to 0 we can
                # shift all the entries to the right if items are prepended
                self._store.roll(-new_min)
