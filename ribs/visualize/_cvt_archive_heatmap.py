...
def cvt_archive_heatmap(
    ...
    *,
    ...
    if archive.measure_dim == 1:
        # Read in pcm kwargs -- linewidth and edgecolor are overwritten by our args.
        ...
        # centroid_sort_idx tells us which index to place the centroid at such that it
        # is sorted, i.e., it maps from the indices in the centroid array to the cell
        # indices. This means that if you index with it, e.g., arr[centroid_sort_idx],
        # you get a sorted array.
        #
        # For computing cell_objectives, we need to know the inverse mapping, i.e., the
        # mapping from cell indices to centroid indices. This way, when we index with
        # it, we get the original order of centroids. This original order then matches
        # with the objectives in objective_batch.
        ...
        vertices = []
        # The facecolor of each cell. Shape (n_regions, 4) for RGBA format, but we do
        # not know n_regions in advance.
        facecolors = []
        # Boolean array indicating which of the facecolors needs to be computed with the
        # cmap. The other colors correspond to empty cells. Shape (n_regions,)
        facecolor_cmap_mask = []
        # The objective corresponding to the regions which must be passed through the
        # cmap. Shape (sum(facecolor_cmap_mask),)
        facecolor_objs = []
        ...
        for region, objective in zip(vor.regions, region_obj):
            ...
            if clip:
                # Clip the cell vertices to the polygon. Clipping may cause some cells
                # to split into two or more polygons, especially if the clip polygon has
                # holes.
        ...
        # Plot the collection on the axes. Note that this is faster than plotting each
        # polygon individually with ax.fill().
        ax.add_collection(
