...
def test_threshold_update_for_multiple_cells(learning_rate, _compute_thresholds):
    ...
    result_true = np.repeat(
        [
            calc_expected_threshold(
                objective[5 * i : 5 * (i + 1)], cur_threshold[5 * i], learning_rate
            )
            for i in range(3)
        ],
        5,
...
def test_init_learning_rate_and_threshold_min():
    ...
    with pytest.raises(ValueError):
        GridArchive(
            solution_dim=2, dims=[10, 20], ranges=[(-1, 1), (-2, 2)], threshold_min=0
        )
    ...
    with pytest.raises(ValueError):
        GridArchive(
            solution_dim=2, dims=[10, 20], ranges=[(-1, 1), (-2, 2)], learning_rate=0.1
        )
