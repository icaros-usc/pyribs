...
def add_random_sphere(archive, x_range, y_range, use_objective=False):
    ...
    for _ in range(10):
        ...
        objective = -np.sum(np.square(solutions), axis=1) if use_objective else None
...
def proximity_archive_2d():
    ...
    archive = ProximityArchive(
        solution_dim=2, measure_dim=2, k_neighbors=5, novelty_threshold=0.1, seed=42
    )
...
def test_2d_custom_axis(proximity_archive_2d):
    ...
    proximity_archive_plot(
        proximity_archive_2d, ax=ax, cmap=[[0.5, 0.5, 0.5]], cbar=None
    )
...
def test_2d_long_square(proximity_archive_2d_long):
    plt.figure(figsize=(8, 6))
    proximity_archive_plot(
        proximity_archive_2d_long, aspect="equal", cmap=[[0.5, 0.5, 0.5]], cbar=None
    )
