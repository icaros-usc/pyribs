...
from .conftest import (
    add_uniform_sphere_1d,
    add_uniform_sphere_2d,
...
def grid_archive_2d():
    ...
    # is correct, yet high-res enough that we can see different colors.
    archive = GridArchive(
        solution_dim=2, dims=[10, 10], ranges=[(-1, 1), (-1, 1)], seed=42
    )
...
def grid_archive_2d_long():
    ...
    archive = GridArchive(
        solution_dim=2, dims=[10, 10], ranges=[(-2, 2), (-1, 1)], seed=42
    )
...
def grid_archive_3d():
    ...
    archive = GridArchive(
        solution_dim=3, dims=[10, 10, 10], ranges=[(-2, 2), (-1, 1), (-2, 1)], seed=42
    )
...
def test_listed_cmap(grid_archive_2d):
    ...
    plt.figure(figsize=(8, 6))
    grid_archive_heatmap(grid_archive_2d, cmap=[[1, 0, 0], [0, 1, 0], [0, 0, 1]])
...
def test_boundaries(grid_archive_2d):
    plt.figure(figsize=(8, 6))
    grid_archive_heatmap(
        grid_archive_2d, pcm_kwargs={"edgecolor": "black", "linewidth": 0.1}
    )
